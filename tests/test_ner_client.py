...
class TestNerrClient(unittest.TestCase):
	...
	def test_get_ents_returns_dictionary_given_non_empty_string_causes_empty_spacy_doc_ents(self):
		model = NerModelTestDouble("eng")
		model.returns_doc_ents([])
		ner = NamedEntityClient(model)
	...
	def test_get_ents_given_multiple_events_serializes_all(self):
		model = NerModelTestDouble("eng")
